...
function load(){try{var x=JSON.parse(localStorage.getItem(KEY));if(x&&x.users){if(!x.settings)x.settings={banner:"",maintenance:false,regOpen:true,inviteOnly:false,wordFilter:"",welcome:"Welcome to Pulse",theme:"dark",skin:"classic"};if(!x.settings.theme)x.settings.theme="dark";if(!x.settings.skin)x.settings.skin="classic";if(!x.audit)x.audit=[];if(!x.reports)x.reports=[];return x}}catch(e){}var d=seed();save(d);return d}
